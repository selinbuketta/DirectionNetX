...
def direction_loss(v_pred, v_true, eps=1e-8):
    """Safe cosine similarity loss avoiding NaNs."""
    v_pred = tf.math.l2_normalize(v_pred, axis=-1, epsilon=eps)
    v_true = tf.math.l2_normalize(v_true, axis=-1, epsilon=eps)
    loss = -tf.reduce_mean(tf.reduce_sum(v_pred * v_true, axis=-1))
    return loss



  #For mock dataset commented out
